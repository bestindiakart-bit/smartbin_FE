...
//                       maxWidth: colWidth,
//                     }}
//                     className="relative p-4 sticky top-0 bg-[#f8fafc] border-r border-b border-slate-200 group"
//                   >
...
//               {/* ACTIONS HEADER */}
//               {showActions && (
//                 <th className="p-4 w-[72px] min-w-[72px] max-w-[72px] text-[13px] font-bold text-slate-600 text-center sticky top-0 right-0 bg-[#f8fafc] border-l border-b border-slate-200">
...
//                     } else {
//                       const cellValue = stringifyValue(row[col.key]);
//                       content = (
//                         <span
//                           title={cellValue}
...
const ReUsable_Table = ({
  ...
  useEffect(() => {
    const initialWidths = {};
    columns.forEach((col) => {
      initialWidths[col.key] = col.width || 180;
    });
    setColWidths(initialWidths);
  }, [columns]);
  ...
  const setColumnSearch = (key, value) => {
    setSearchQueries((prev) => ({ ...prev, [key]: value }));
  };

  const renderSearchBox = (col) => (
    <div
      className="relative group text-left w-full"
      onClick={(e) => e.stopPropagation()}
    >
  ...
  const groupedHeaderRows = useMemo(() => {
    if (!hasGroupedHeaders) return null;
    ...
    while (index < columns.length) {
      ...
      if (!col.group) {
        groups.push({ type: "single", column: col });
        index += 1;
        continue;
      }
    ...
    return groups;
  }, [columns, hasGroupedHeaders]);

  const renderColumnHeader = (col, extraClass = "") => {
    const colWidth = colWidths[col.key] || col.width || 180;

    return (
      <th
        key={col.key}
        rowSpan={hasGroupedHeaders ? col.rowSpan || 1 : undefined}
        style={{
          width: colWidth,
          minWidth: colWidth,
          maxWidth: colWidth,
        }}
        className={`relative p-4 bg-[#f8fafc] border-r border-b border-slate-200 group ${extraClass}`}
      >
        <div className="flex flex-col gap-2 items-center text-center w-full px-2">
          <span className="text-[13px] font-bold text-slate-600 whitespace-nowrap uppercase tracking-wider text-center w-full truncate">
            {col.header}
  ...
  const renderCellContent = (row, col) => {
    if (col.key === "currentStatus") {
      return renderTextStatusBadge(row[col.key]);
    }

    if (col.isStatus && showbinstatus) {
      return (
        <div className="flex items-center justify-center">
          <StatusIndicator
            value={row[col.key]}
    ...
    if (col.isPaid && showWhstatus) {
      return (
        <div className="flex items-center justify-center">
          <StatusIndicator
            value={row[col.key]}
            message={row.warehouseStatusMessage || row.binStatusMessage}
          />
        </div>
      );
    }

    if (col.isToggle && showToggle) {
      return (
        <div className="flex items-center justify-center">
          <motion.button
            whileTap={{ scale: 0.9 }}
    ...
    if (col.isQtyIndicator && showQtyStatus) {
      return (
        <div className="flex items-center justify-center">
          <motion.div
            whileHover={{ scale: 1.3 }}
    ...
    return (
      <span
        title={cellValue}
        className={`block w-full text-center truncate px-2 ${
          isInactive ? "opacity-70 text-slate-400" : "text-slate-700"
        }`}
      >
        {cellValue}
      </span>
            ...
            {hasGroupedHeaders ? (
              <>
                <tr className="bg-[#f8fafc]">
                  <th
                    rowSpan={2}
                    className="p-4 w-12 min-w-[48px] max-w-[48px] text-center bg-[#f8fafc] z-50 border-r border-b border-slate-200"
                  >
                    <div className="flex items-center justify-center">
                      <input
                        type="checkbox"
                  ...
                  {groupedHeaderRows.map((group, index) => {
                    if (group.type === "single") {
                      return renderColumnHeader(group.column, "z-40");
                    }
                    ...
                    return (
                      <th
                        ...
                        className="p-3 z-40 bg-[#f8fafc] border-r border-b border-slate-200 text-center"
                      >
                  ...
                  {showMoreView && (
                    <th
                      rowSpan={2}
                      className="p-4 w-24 min-w-[96px] max-w-[96px] text-[13px] font-bold text-slate-600 text-center bg-[#f8fafc] z-40 border-r border-b border-slate-200"
                    >
                      More Info
                    </th>
                  )}
                  ...
                  {showActions && (
                    <th
                      rowSpan={2}
                      className="p-4 w-[72px] min-w-[72px] max-w-[72px] text-[13px] font-bold text-slate-600 text-center bg-[#f8fafc] z-50 border-l border-b border-slate-200"
                    >
                      {ActionChildren || "Actions"}
                    </th>
                  )}
                ...
                <tr className="bg-[#f8fafc]">
                  {columns
                    ...
                    .map((col) => renderColumnHeader(col, "z-40"))}
                </tr>
              </>
            ) : (
              <tr className="bg-[#f8fafc]">
                <th className="p-4 w-12 min-w-[48px] max-w-[48px] text-center bg-[#f8fafc] z-50 border-r border-b border-slate-200">
                  <div className="flex items-center justify-center">
                    <input
                      type="checkbox"
                      checked={
                ...
                {showMoreView && (
                  <th className="p-4 w-24 min-w-[96px] max-w-[96px] text-[13px] font-bold text-slate-600 text-center bg-[#f8fafc] z-40 border-r border-b border-slate-200">
                    More Info
                  </th>
                )}

                {showActions && (
                  <th className="p-4 w-[72px] min-w-[72px] max-w-[72px] text-[13px] font-bold text-slate-600 text-center bg-[#f8fafc] z-50 border-l border-b border-slate-200">
                    {ActionChildren || "Actions"}
                  </th>
                )}
              </tr>
          ...
          <tbody>
            {filteredData.map((row, index) => {
              ...
              return (
                ...
                >
                  <td
                    className={`p-4 w-12 min-w-[48px] max-w-[48px] text-center z-20 border-r border-slate-200 ${rowBorderClass} transition-colors ${
                      isSelected
                        ? "bg-[#f0f9ff]"
                        : isInactive
                        ? "bg-[#fafafa]"
                  ...
                  {columns.map((col, idx) => {
                    ...
                    return (
                      <td
                        ...
                        className={`p-4 text-[14px] font-medium align-middle border-r border-slate-200 ${rowBorderClass} text-center overflow-visible`}
                      >
                  ...
                  {showActions && (
                    <td
                      className={`p-4 w-[72px] min-w-[72px] max-w-[72px] text-center z-20 border-l border-slate-200 ${rowBorderClass} transition-colors ${
                        isSelected
                          ? "bg-[#f0f9ff]"
                          : isInactive
                          ? "bg-[#fafafa]"
